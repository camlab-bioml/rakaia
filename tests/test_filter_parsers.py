...
def test_basic_filter_parsers():
    ...
    fil_type, fil_val, fil_sigma, chan_color = get_current_channel_blend_params(blend_dict, "channel_1")
    ...
    assert get_current_or_default_filter_param("median", "gaussian") == "gaussian"
    assert isinstance(get_current_or_default_filter_param("gaussian", "gaussian"), dash._callback.NoUpdate)

    assert get_current_or_default_channel_color({'hex': '#FFFFFF'}, '#FF0000') == {'hex': '#FF0000'}
    assert isinstance(get_current_or_default_channel_color({'hex': '#FF0000'}, '#FF0000'), dash._callback.NoUpdate)


def test_filter_parsers_with_preset():
    to_apply_filter, filter_type_return, filter_val_return, filter_sigma_return, color_return = \
        get_current_default_params_with_preset(None, None, None, None)
    ...
    to_apply_filter, filter_type_return, filter_val_return, filter_sigma_return, color_return = \
        get_current_default_params_with_preset("gaussian", 5, 0.8, '#FF0000')

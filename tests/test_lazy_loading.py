...
def test_lazy_loading_single_marker_mcd(get_current_dir):
    ...
    assert parse_files_for_lazy_loading(session_config, 'query+++slide1+++Xylene_5')
    selection = ['Ir191']
    single_load = SingleMarkerLazyLoader(uploaded_dict, 'query+++slide1+++Xylene_5',
                                         session_config, selection, 55, '+++', 'int').get_image_dict()
    assert all(single_load['query+++slide1+++Xylene_5'][elem] is None for elem in
               single_load['query+++slide1+++Xylene_5'].keys() if elem not in selection)
    assert all(isinstance(single_load['query+++slide1+++Xylene_5'][elem], np.ndarray) for elem in
               single_load['query+++slide1+++Xylene_5'].keys() if elem in selection)
...
def test_lazy_loading_single_marker_spatial(get_current_dir):
    ...
    assert parse_files_for_lazy_loading(session_config, 'visium_thalamus+++slideNA+++acq')
    ...
    lazy_load = SingleMarkerLazyLoader(visium_parser.image_dict, 'visium_thalamus+++slideNA+++acq',
                    session_config, 'Sox17')
    single_load = lazy_load.get_image_dict()
    dims = lazy_load.get_region_dim()
    assert all(single_load['visium_thalamus+++slideNA+++acq'][elem] is None for elem in
               single_load['visium_thalamus+++slideNA+++acq'].keys() if elem != 'Sox17')
    assert all(isinstance(single_load['visium_thalamus+++slideNA+++acq'][elem], np.ndarray) for elem in
               single_load['visium_thalamus+++slideNA+++acq'].keys() if elem == 'Sox17')
...
def test_lazy_loading_single_marker_tiff(get_current_dir):
    ...
    assert all(uploaded_dict['for_recolour+++slideNA+++acq'][elem] is None for elem in
               uploaded_dict['for_recolour+++slideNA+++acq'].keys())
    lazy_loader = SingleMarkerLazyLoader(uploaded_dict, 'for_recolour+++slideNA+++acq',
                                         uploads, 'channel_1')
    single_load = lazy_loader.get_image_dict()
    dims = lazy_loader.get_region_dim()
    assert all(isinstance(single_load['for_recolour+++slideNA+++acq'][elem], np.ndarray) for elem in
               single_load['for_recolour+++slideNA+++acq'].keys())

...
def test_quant_random_forest(get_current_dir):
    ...
    with_predictions = QuantificationRandomForest(measurements_csv, "area_use", "area_predict").quantification_with_labels()
    assert 'area_predict' in pd.DataFrame(with_predictions).columns
    assert 'Unassigned' not in pd.DataFrame(with_predictions)['area_predict']
...
def test_run_quant_models(get_current_dir):
    ...
    rf_model = run_quantification_model(leiden_as_model, "area", "area_predict", "random forest")

...
flagged due to security reasons. Your computer may state that the application cannot be opened,
and/or recommends moving the file to the `Trash`. If the application fails to start, users
should check the security settings to see if `rakaia` is flagged from an unknown developer:
https://support.apple.com/en-gb/guide/mac-help/mh40616/mac. Under `Settings` -> `Privacy & Security` -> `Security`, the rakaia application should have the option to `Open anyway`. Users should enable this option, and the file will either re-open, or the user can re-execute the file.

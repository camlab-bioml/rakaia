...
def cli_parser():
    parser = argparse.ArgumentParser(add_help=False,
            ...
            usage='Example:\n python apply_blend.py -i first.mcd second.mcd -o out_images/ -p param.json')
...
def main(sysargs=sys.argv[1:]):
    ...
    for file in files_to_process:
        ...
        initial_parse = {key: value for key, value in
                          FileParser([str(file)], lazy_load=use_lazy_load).image_dict.items() if key not in
        ...
        for roi in list(initial_parse.keys()):
            try:
                ...
                print("\033[31m" + "ERROR: The blend parameter JSON passed appears to be either not be the expected rakaia JSON output format, or "
                              "for a different panel than the files to be processed. Please carefully review the CLI inputs.")
                sys.exit(1)
            # use the acquisition id unless it's not unique
            exp, slide, acq = split_string_at_pattern(roi)
            file_out_name = acq if acq != 'acq' else exp
            if args.verbose:
            ...
            additive_image = apply_filter_to_array(additive_image, blend_params['config']['filter']['global_apply_filter'],

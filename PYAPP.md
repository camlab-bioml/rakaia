# Building rakaia runtime installers with pyapp

**NOTE: The developers of rakaia recommend building standalone package distributions
with `pyinstaller` using the [official build documentation](./BUILDING.md). Creating a runtime
bootstrap with pyapp should only be attempted if building with pyinstaller is either not possible, or unsuccessful.**
